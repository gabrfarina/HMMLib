...
namespace hmmlib {
    ...
    HMM<float_type, sse_float_type>::forward(const sequence& obsseq,
        ...
        HMMMatrix <float_type, sse_float_type>& T_t = *(new HMMMatrix<float_type, sse_float_type>(T.get_no_columns(),
                                                                                                  T.get_no_rows()));
        T.transpose(T_t);
    ...
    HMM<float_type, sse_float_type>::viterbi(const sequence& obsseq, sequence& hiddenseq) {
        ...
        boost::shared_ptr <HMMMatrix<float_type, sse_float_type>> T_t_ptr(
                new HMMMatrix<float_type, sse_float_type>(T.get_no_columns(), T.get_no_rows()));
        boost::shared_ptr <HMMMatrix<float_type, sse_float_type>> T_t_log_ptr(
                new HMMMatrix<float_type, sse_float_type>(T.get_no_rows(), T.get_no_columns()));
        HMMMatrix <float_type, sse_float_type>& T_t_log = *T_t_log_ptr;

        boost::shared_ptr <HMMMatrix<float_type, sse_float_type>> E_log_ptr(
                new HMMMatrix<float_type, sse_float_type>(E.get_no_rows(), E.get_no_columns()));
      ...
      #pragma omp parallel for
            ...
            for (int s = 0; s < no_states; ++s) { // fill path_probs(i,s)
                ...
                for (int c = 0;
                     c < no_chunks - 1; ++c) { // no_chunks-1 since we don't want to take the max of some garbage.
